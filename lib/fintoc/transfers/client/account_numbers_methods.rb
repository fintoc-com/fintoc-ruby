...
module Fintoc
  module Transfers
    module AccountNumbersMethods
      def create_account_number(account_id:, description: nil, metadata: nil, **params)
      ...
      def list_account_numbers(**params)
        _list_account_numbers(**params).map { |data| build_account_number(data) }
      end

      def update_account_number(account_number_id, **params)
      ...
      def _create_account_number(account_id:, description: nil, metadata: nil, **params)
        ...
        post(version: :v2).call('account_numbers', **request_params)
      end

      def _get_account_number(account_number_id)
        get(version: :v2).call("account_numbers/#{account_number_id}")
      end

      def _list_account_numbers(**params)
        get(version: :v2).call('account_numbers', **params)
      end

      def _update_account_number(account_number_id, **params)
        patch(version: :v2).call("account_numbers/#{account_number_id}", **params)
      end

      def build_account_number(data)
        Fintoc::Transfers::AccountNumber.new(**data, client: self)
      end
    end
  end
end
